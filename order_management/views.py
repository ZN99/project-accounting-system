...
def project_list(request):
    ...
    # 利益フィルタ (gross_profit) - データベースレベルでフィルタ
    ...
    if profit_max in ['None', '', None]:
        profit_max = None
    # キャッシュされたgross_profitフィールドでフィルタ（高速）
    if profit_min:
        projects = projects.filter(gross_profit__gte=Decimal(str(profit_min)))
    if profit_max:
        projects = projects.filter(gross_profit__lte=Decimal(str(profit_max)))
